...
public class ChartDataBuilder {
	...
	public String stackedAreaChartJson(int yValue, String start, String end) throws SQLException{
		String content =  null;
		statement = DBConn.prepareStatement("select * from artifact_type order by artifact_type_id;");
		ResultSet artifactTypeResult = statement.executeQuery();
		String subQuery = "";
		if(start !=null && end!= null){
			subQuery = " and artifact.date_created > '"+start+"' and artifact.date_last_modified <'"+end+"' ";
		}
		JSONArray mainList = new JSONArray();
		while(artifactTypeResult.next()){
			JSONObject obj = new JSONObject();
			switch(yValue){
			case 1:
				statement = DBConn.prepareStatement("select unix_timestamp((review.date_created))*1000 start,"
						...
						+ " where artifact.artifact_type_id = ? "+subQuery+" group by review.story_name order by review.date_created;");
				break;
			case 2:
				statement =  DBConn.prepareStatement("select unix_timestamp((review.date_created))*1000 start,"
						...
						+ " inner join review on review.review_id = artifact.review_id where artifact.artifact_type_id = ?  "+subQuery+" "
						+ " group by review.story_name order by review.date_created;");
				break;
			case 3:
				statement = DBConn.prepareStatement("select unix_timestamp((review.date_created))*1000 start,"
						+ " count(distinct artifact_version.version_no) 'No. Of Versions per User Story' from artifact"
						+ " inner join artifact_version on artifact_version.artifact_id = artifact.artifact_id"
						+ " inner join review on review.review_id = artifact.review_id  where artifact.artifact_type_id = ?  "+subQuery+" "
						+ "  group by review.story_name order by review.date_created;");
				break;
			}
